...
if (! function_exists('aerospace_citation')) :
    ...
    function aerospace_citation()
    {
        ...
        printf('<span class="meta-label">Cite this Page</span><p><span class="citation">' . esc_html('%1$s,', 'aerospace') . ' <em>%2$s</em> ' . esc_html('(Washington D.C.: Center for Strategic and International Studies, %3$s), %4$s', 'aerospace') . '</span><button id="btn-copy" class="btn btn-gray" data-clipboard-target=".citation" aria-label="Copied!">Copy</button></p>', $authors, get_the_title(), $year, get_the_permalink()); // WPCS: XSS OK.
    }
endif;
...
if (! function_exists('aerospace_post_sources')) :
    ...
    function aerospace_post_sources($id)
    {
        ...
        if (in_array($post_type, array( 'post', 'aerospace101', 'data' ), true)) {
            ...
            if (in_array($post_type, array( 'aerospace101', 'data' )) || is_page_template('single-longform.php')) {
                $colmd = 'col-md-12';
    ...
    function aerospace_post_footnotes()
    {
        if (class_exists('easyFootnotes')) {
            ...
            if (in_array($post_type, array( 'post', 'aerospace101', 'data' ), true) && $footnotes != '') {
                ...
                printf('<div class="entry-footnotes col-xs-12 %2$s%4$s"><h4 class="subheading">%3$s' . esc_html('Footnotes', 'aerospace') . '</h4><div class="collapsible-content"><ol class="easy-footnotes-wrapper">%1$s</ol></div></div>', $footnotes, $colmd, $icon, $collapsible); // WPCS: XSS OK.
            }
        }
    }
endif;
...
if (! function_exists('aerospace_event_location')) :
        ...
        function aerospace_event_google_map($id, $echo = true)
        {
            if ('events' === get_post_type()) {
                $address = get_post_meta($id, '_events_address', true);
                $csis_map = 'https://www.google.com/maps/place/1616+Rhode+Island+Ave+NW,+Washington,+DC+20036/@38.90651,-77.0398798,17z';
                if ('' !== $address) {
                    ...
                    if ($echo) {
                        printf('<a href="%1$s" class="btn btn-xs btn-gray btn-map" target="_blank" rel="nofollow">' . esc_html('Map', 'aerospace') . '</a>', $csis_map); // WPCS: XSS OK.
                    } else {
                        return sprintf('<a href="%1$s" class="btn btn-xs btn-gray btn-map" target="_blank" rel="nofollow">' . esc_html('Map', 'aerospace') . '</a>', $csis_map); // WPCS: XSS OK.
                    }
                }
            }
        }
