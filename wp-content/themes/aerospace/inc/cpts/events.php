...
function events_build_meta_box( $post ){
	...
	$current_is_featured = get_post_meta( $post->ID, '_events_is_featured', true );
	...
	<div class='inside'>
		...
		<h3><?php _e( 'Sponsored By', 'aerospace' ); ?></h3>
		<p>
			<input type="text" class="large-text" name="sponsored_by" value="<?php echo $current_sponsored_by; ?>" />
		</p>

		<h3><?php _e( 'Event Dates', 'aerospace' ); ?></h3>
		<p>
			<label for="start_date"><?php _e( 'Start Date:', 'aerospace' ); ?></label> <input type="date" class="medium-text" name="start_date" value="<?php echo esc_textarea( $current_start_date ); ?>" /><br />
			<label for="end_date"><?php _e( 'End Date:', 'aerospace' ); ?></label> <input type="date" class="medium-text" name="end_date" value="<?php echo esc_textarea( $current_end_date ); ?>" />
		</p>

		<h3><?php _e( 'Time', 'aerospace' ); ?></h3>
		<p>
			<input type="text" class="medium-text" name="time" value="<?php echo $current_time; ?>" />
		</p>

		<h3><?php _e( 'Address', 'aerospace' ); ?></h3>
		<p>
			<input type="text" class="large-text" name="address" value="<?php echo $current_address; ?>" />
		</p>

		<h3><?php _e( 'Event Video URL', 'aerospace' ); ?></h3>
		<p>
			<input type="text" class="large-text" name="video_url" value="<?php echo $current_video_url; ?>" />
		</p>
...
function events_save_meta_box_data( $post_id ){
	...
	if ( isset( $_REQUEST['is_featured'] ) ) {
		update_post_meta( $post_id, '_events_is_featured', sanitize_text_field( $_POST['is_featured'] ) );
	}
