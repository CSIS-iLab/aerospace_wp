<?php
/**
 * The template for displaying single posts.
 *
 * @link https://developer.wordpress.org/themes/basics/template-hierarchy/#single-post
 *
 * @package Aerospace
 */

get_header(); ?>

    <div id="primary" class="content-area">
        <main id="main" class="site-main content-wrapper">

    		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			    <header class="entry-header row">
			    	<div class="post-format-container col-xs-12">
			    		Post Format
			    		Categories
			    	</div>
			    	<div class="title-container col-xs-12">
				    	<?php the_title('<h1 class="entry-title">', '</h1>'); ?>
				    	<div class="entry-meta">
				        	<?php aerospace_posted_on(); ?>
				        	Authors
				        </div>
				    </div>
			    </header><!-- .entry-header -->

			    <div class="entry-content">
			    	Featured Image
			    	Highlights
			    <?php
			    the_content(
			        sprintf(
			            wp_kses(
			                /* translators: %s: Name of current post. Only visible to screen readers */
			                    __('Continue reading<span class="screen-reader-text"> "%s"</span>', 'aerospace'),
			                array(
			                        'span' => array(
			                            'class' => array(),
			                        ),
			                    )
			            ),
			            get_the_title()
			        ) 
			    );
			    ?>
			    </div><!-- .entry-content -->

			    <footer class="entry-footer">
			    	<div class="citation-share-container row">
			    		<div class="entry-citation col-xs-12 col-md-9">
			    			Citation
			    		</div>
			    		<div class="entry-share col-xs-12">
			    			<?php get_template_part( 'share-inline' ); ?>
			    		</div>
			    	</div>
			    	<div class="footnotes-sources-container row">
			    		<div class="entry-footnotes col-xs-12 col-md-6">
			    			Footnotes
			    		</div>
			    		<div class="entry-sources col-xs-12 col-md-6">
			    			Sources
			    		</div>
			    	</div>
			    	<div class="authors-container row">
			    		<?php esc_html_e( 'About the Authors', 'aerospace' ); ?>
			    		Authors
			    	</div>
			    	<div class="explore-more-container row">
			    		<?php esc_html_e( 'Explore More', 'aerospace' ); ?>
			    		Tags<br />
			    		Related Posts
			    	</div>
			    </footer><!-- .entry-footer -->
			</article><!-- #post-<?php the_ID(); ?> -->


        </main><!-- #main -->
    </div><!-- #primary -->

<?php
get_footer();
